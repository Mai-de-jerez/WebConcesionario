        ...
        <div class="table-container">
            <table id="tablaUsuarios">
                ...
                <tbody>
                    ...
                    %>
	                    <tr>
	                    	<td style="font-weight: bold; color: #764ba2;"><%= u.getId_usuario() %></td>
	                        <td><strong><%= u.getUsuario() %></strong></td>
	                        <td><%= u.getEmail() %></td>
	                        <td><%= u.getRol() %></td>
	                        <td><%= u.getRol().getNivel() %></td> 						
							<td class="actions" style="text-align: center;">
							    <a href="DetalleUsuario?id=<%= u.getId_usuario() %>" class="row-btn btn-success btn-small">Ver más</a>
							
							    <%-- Solo si es Admin Y no es él mismo, ve el botón de borrar --%>
							    <% if (esAdmin) { 
							        boolean noSoyYo = (u.getId_usuario() != usuLogueado.getId_usuario());
							        if (noSoyYo) { %>
							            <a href="EliminarUsuario?id=<%= u.getId_usuario() %>" 
